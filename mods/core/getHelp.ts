...
const getHelp = (message: Discord.Message, args: string[], bot: Bot) => {
    if (args && args[0]) {
        ...
        const mod = bot.cmdMgr.get(args[0])
        if (!mod) return
        ...
        message.channel.send({ embeds: [
            new Discord.MessageEmbed()
                ...
                .addField(`Command: \`\` ${bot.cmdMgr.getCommands(mod).join(', ')} \`\``, `Aliases: \`\` ${bot.cmdMgr.getAliases(mod).join(', ')} \`\``)
                .addField('Usage', `${mod.usage
                    .replace(/%prefix%+/, bot.config.prefix)           
                 }`)
