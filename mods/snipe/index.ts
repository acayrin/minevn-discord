...
const SnipeDelete = (message: Message, args: any, bot: Bot) => {
	...
	// debug
	if (bot.debug)
		bot.logger.debug(`[Snipe] Deleted +${message.id} (${record_D[message.channelId].length}/${bot.config.snipe.limit})`)
...
const SnipeUpdate = (oldMsg: Message, newMsg: Message, bot: Bot) => {
	...
	// debug
	if (bot.debug)
		bot.logger.debug(`[Snipe] Updated +${oldMsg.id} (${record_U[oldMsg.channelId].length}/${bot.config.snipe.limit})`)
