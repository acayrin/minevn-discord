...
class IPC extends EventEmitter {
	...
	private _events = new Map<string, Yujin.IPCEvent[]>();
	constructor() {
		super();
	...
	private eventListenerStart(): void {
		process.on('message', (msg: Yujin.IPCEvents) => {
	 ...
	 */
	public register(event: string, callback: (msg: Yujin.IPCEvents) => void): Yujin.IPCEventListener {
		if (!this._events.get(event)) this._events.set(event, []);
		const cb: Yujin.IPCEvent = {
			cb: callback,
		};
	 ...
	 */
	public unregister(
		event: Yujin.IPCEventListener | string,
		callback?: ((msg: Yujin.IPCEvents) => void | number) | number,
	): void {
