						...
						return message.reply(
							{
								embed: new Eris.Embed()
									.setTitle(`${target.tag()} has been triggered`)
									.setImage(
										`https://some-random-api.ml/canvas/triggered?avatar=${target.getAvatar('png')}`,
									),
							},
							{
								name: 'pic.png',
				...
				},
				{
					...
					process: async (i, o) => {
						...
						return i.createFollowup(
							{
								embeds: [new Eris.Embed().setTitle(`${target.tag()} has been triggered`)],
							},
							{
								name: 'pic.gif',
								file: await (
