 ...
 */
export async function ChatFilter(message: Eris.Message, mod: Yujin.Mod): Promise<unknown> {
	...
	// payload: embeds
	payload.embeds = payload.embeds.concat(message.embeds);
	...
	message
		...
		.finally(async () => {
			if (mod.getConfig().method?.toLowerCase() === 'webhook' && message.channel.type === 0) {
